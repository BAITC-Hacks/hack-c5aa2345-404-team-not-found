...
**Нативный Windows EXE на C# / WinForms собран и запущен. Проверены демопротокол, системный выбор файла, сохранение DOCX и закрытие приложения; 21/21 проверок native-кода прошли. Backend остаётся каркасом, сквозной AI-пайплайн не интегрирован.**
...
| Local AI | отдельная задача участника по моделям и адаптерам | проверка реальной русско-казахской записи, интеграция с backend |
...
Аудио, транскрипты и результаты не передаются в облачные AI API. После первоначальной загрузки моделей целевой пайплайн должен работать без внешнего интернета; это ещё нужно проверить на реальной записи.
...
| Распознавание | faster-whisper, Whisper large-v3 / turbo | локальный текст и временные метки |
| Говорящие | pyannote.audio 4.x, Community-1 | локальное разделение по говорящим |
| Анализ | локальная Qwen3 через Ollama / llama.cpp | резюме и поручения |
...
Параметры моделей и версии для полного запуска фиксирует участник Local AI после проверки железа. В native EXE автономный демопротокол можно сохранить в DOCX. Для реального результата DOCX/PDF запрашиваются у backend; автономный PDF предусмотрен через системную печать в PDF при наличии соответствующего принтера, но печать пока не проверена. Таблица поручений доступна только для просмотра: редактирование и сохранение задач не реализованы, метода для них нет в текущем API-контракте.
...
У сохранённого Streamlit UI ранее прошли 14 тестов; это отдельный результат, не часть 21 native-проверки. Реальные модели, обработка записи и промышленный backend ещё не проверены.
