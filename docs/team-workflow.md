...
- Backend интегрирует очередь, хранение, модели и экспорт. API, очередь и экспорт реализованы; финальный GPU-прогон
  через EXE ещё не подтверждён. Запуск — docs/backend-launch.md.
