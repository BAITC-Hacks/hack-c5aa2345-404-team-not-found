...
Активный backend пока возвращает status=scaffold и 501 на загрузку. Покажи эту неготовность явно. Пока моделей нет, отдельная кнопка может открывать синтетический пример с видимой пометкой деморежима. Ошибки сервера не маскируй демоданными. Процент прогресса не придумывай, показывай stage/status.
...
Проверь компиляцию и реальный запуск итогового native EXE. Проверь настройки и перезапуск, отключённый сервер/scaffold, явное демо, выбор файла, состояния, результат и экспорт. Различай тесты с заглушками и живой backend. Старые 14 Streamlit-тестов не являются проверкой нового WinForms-клиента. Если часть сценария блокируется каркасом API, запиши это и продолжай независимые проверки.
