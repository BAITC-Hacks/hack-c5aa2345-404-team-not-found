...
**Статус: контракт для реализации.** Сейчас реализован только scaffold health; `POST /api/meetings` возвращает 501. Остальные методы ниже пока не реализованы. Документ и пример позволяют разрабатывать Windows-клиент независимо. Владелец контракта — backend (Medet). Переход с прежнего мобильного плана на EXE не меняет URL и схемы HTTP API.
...
## Что доступно в текущем сервере

Исходник маршрутов: [backend/app/api/routes.py](../backend/app/api/routes.py).

| Запрос | Фактический ответ |
| --- | --- |
| `GET /health` | 200, `status: "scaffold"`, `local_only: true`, planned-модули |
| `POST /api/meetings` | 501; файл не сохраняется, модели не вызываются |
| `GET /api/meetings` | 405: путь есть только для POST, список ещё не реализован |
| Методы ниже с ID встречи в пути | Пока отсутствуют, возвращают 404 |

Значения `.env.example` не читаются этим каркасом автоматически. Установка
локальных моделей сама по себе не изменяет доступные маршруты.

## Целевые методы после реализации

| Метод | Запрос | Успешный ответ |
| --- | --- | --- |
| `GET /health` | — | 200, текущие `status`, `local_only`, `processing_modules`; доступность health ещё не доказывает готовность AI |
...
Целевые форматы MVP: WAV, MP3, M4A, MP4. Реализация backend должна проверять
содержимое и лимит размера, не доверяя имени файла; в заглушке эти проверки ещё
не выполняются. Предусмотренные ошибки: 413 (слишком большой файл),
415 (неподдерживаемый формат), 422 (неверные поля), 404 (неизвестный ID),
503 (сервис обработки недоступен). Повторный POST создаёт новую встречу:
клиент не повторяет загрузку автоматически при неопределённом результате сетевого запроса.

Методы изменения поручений, статусов задач и удаления встреч в v1 не определены.
Клиент не должен вызывать прежний Streamlit PUT как часть этого контракта.
...
Прикладная HTTP-ошибка: `{"detail": {"code": "not_ready", "message": "Результат ещё не готов"}}`. При failed клиент читает `Meeting.error`. При этом FastAPI может возвращать стандартный массив `detail` для 422, а текущий scaffold — строковый `detail` для 501; клиент должен безопасно отображать все три формы.
