# Согласованный целевой API v1 для Android MVP

**Статус: контракт для реализации.** Сейчас работает только scaffold health; `POST /api/meetings` возвращает 501. Остальные методы ниже пока не реализованы. Документ и пример позволяют разрабатывать мобильный клиент независимо. Владелец контракта — backend (Medet).

Base URL задаётся в настройках приложения, например `http://<server-lan-ip>:8000`. Все JSON используют UTF-8, timestamps — ISO 8601 UTC, ID встречи — непрозрачная строка. Время сегментов — секунды от начала записи. APK не вызывает модели напрямую.
...
Деморежим включается явно, помечается на экранах и использует те же типы. Сетевые ошибки никогда не переключают приложение в деморежим автоматически. Настоящие токены Hugging Face и ключи моделей на телефон не передаются.
