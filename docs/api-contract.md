...
Конфигурация читается из переменных окружения при запуске; `.env` автоматически
не читается. [Команды запуска](backend-launch.md).
...
Форматы MVP: WAV, MP3, M4A, MP4. Backend проверяет расширение и сигнатуру,
размер до 250 МБ; затем FFmpeg проверяет декодирование и длительность до 60 минут.
Некорректное декодирование приводит к failed, а не частичному результату. Предусмотренные ошибки: 413 (слишком большой файл),
