...
Реализованы три независимых адаптера: распознавание речи, диаризация и анализ
текста. **В текущем HTTP backend они ещё не подключены:** `/health` сообщает
`scaffold`, создание встречи возвращает `501`. Команды ниже проверяют адаптеры
напрямую и не запускают полный сценарий EXE → API → готовый протокол.
...
firewall** и не доказательство автономности всего EXE/API. Полную проверку
без внешней сети нужно повторить после серверной интеграции.
...
Текст и интервалы говорящих пока выводятся раздельно. Их объединение в единый
транскрипт продукта — незавершённая задача backend. Для WER нужен выверенный текст,
...
14 новых проверок диагностики с синтетикой/моками. Свежий запуск здесь не заявляется.
...
Для полного продукта остаются очередь, хранение, совмещение timestamps,
обработка длинного текста, маршруты результата/экспорта и сквозная проверка с EXE.
...
| EXE получает `501` | Это текущий scaffold API; установка моделей сама по себе не завершает интеграцию |
